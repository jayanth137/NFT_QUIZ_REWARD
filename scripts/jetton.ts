...
export async function run(provider: NetworkProvider): Promise<void> {
    let receiver = await provider.ui().input('input the address of the jetton receiver(default to yourself):');

    const sampleMasterContract = await getMasterContract(provider);
    const sampleContract = await provider.open(
        await Sample.fromInit(sampleMasterContract.address, provider.sender().address!!),
    );
    ...
    const jettonMasterContract = await provider.open(
        await JettonMasterTemplate.fromInit(sampleContract.address, {
            $$type: 'Tep64TokenData',
            flag: BigInt('1'),
            content: 'https://www.jsonkeeper.com/b/WVUO',
        }),
    );
    ...
    const jettonWalletContract = await provider.open(
        await JettonWalletTemplate.fromInit(jettonMasterContract.address, provider.sender().address!!),
    );
